...
export default function Events() {
	...
	const getOrgEvents = useCallback(async () => {
		const [userCookie, roleCookie, accessToken] = await Promise.all([
			getCookie('profile'),
			getCookie('currentRole'),
		...
		console.log('getOrgEvents', { selectedStatus, currentRole, role, status });
		try {
			setLoading(true);
			const res: any = await OrgEventsHttp.getOrgEvents({
				orgId: currentRole.organization.id,
				page: currentPage - 1,
				status,
			});
			if (res.status === 200) {
	...
	return (
		<AdminLayout>
			...
			<div id="admin-events" className="admin-container">
				<EventFilter
